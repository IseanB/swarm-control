...
distance_per_charge = 300  # distance that bot can move
battery_burn = max_charge / distance_per_charge  # the amount of battery cost a movement has
        ...
        for _ in range(num_survivors):
            while True:
                ...
                if (
                    0 <= x < self.size[0]
                    and 0 <= y < self.size[1]
                    and not self.obstacle_collision((x, y))
                ):
...
class Evaluator:
    ...
    def evaluate(self):
        ...
        # Ending Battery
        battery_vals = [robot.battery for robot in self.swarm.actors]
